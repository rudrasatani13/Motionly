...
- Phase 18 smoothing / normalization / confidence filtering — complete.
- Phase 19 joint angle math — complete.
...
## 10o. Joint Angle Calculation Engine (Phase 19)

Phase 19 adds a pure-TypeScript joint-angle layer that consumes the
Phase 18 `ProcessedPoseFrame` and produces a single `AngleSnapshot` per
frame plus a bounded 30-frame ring buffer. It does not count reps,
score form, generate cues, or run any exercise-specific state machine
— those land in Phase 20+.

### Files

| Path                                      | Responsibility                                                                                                                                                                                                                                                                        |
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/types/angles.ts`                     | Angle domain types — `AngleValue`, `BilateralAngleValue`, `AngleMetricValue`, `AngleSnapshot`, `AngleCalculationResult`, `AngleCalculationStats`, `AngleAvailabilityReport`, `AngleHistoryEntry`, `AngleHistoryState`, plus side / unit / name / source-space enums.                  |
| `src/ml/angles/AngleCalculator.ts`        | Pure 2D / 3D vector angle math — `calculateAngle`, `calculateAngle3D`, `angleFromVertical2D`, plus `dot2D` / `dot3D` / `vectorLength2D` / `vectorLength3D` / `safeClamp` / `degreesFromRadians`. Clamps cosine to `[-1, 1]`; never returns `NaN` / `Infinity`.                        |
| `src/ml/angles/JointAngles.ts`            | Named joint helpers — `kneeAngle`, `hipAngle`, `ankleAngle`, `elbowAngle`, `shoulderAngle`, `trunkAngle`, `kneeValgus`, `hipSymmetry`. Vector angles read smoothed image-space landmarks; ratio metrics require Phase 18 normalized landmarks and return typed unavailable otherwise. |
| `src/ml/angles/AngleHistory.ts`           | Bounded ring buffer (default capacity 30). `push` / `latest` / `getAll` / `getRecent` / `clear` / `size` / `capacity` / `toState`. In-memory only, no persistence.                                                                                                                    |
| `src/ml/angles/processAngleSnapshot.ts`   | `AngleFrameProcessor` class owns the history, runs every helper per frame, measures total per-frame overhead, returns an `AngleCalculationResult`. `processAngleSnapshot(processor, processed)` wraps it functionally.                                                                |
| `src/ml/angles/angle-config.ts`           | Angle-layer defaults (`DEFAULT_ANGLE_HISTORY_CAPACITY`, `DEFAULT_ANGLE_VISIBILITY_THRESHOLD`).                                                                                                                                                                                        |
| `src/hooks/usePoseLandmarker.ts`          | Owns a per-session `AngleFrameProcessor` alongside the Phase 18 `PoseFrameProcessor`. Stores the latest `AngleSnapshot` + per-frame angle stats. Resets the processor on stop / unmount / no-pose / model restart.                                                                    |
| `src/store/usePoseStore.ts`               | Extended with `latestAngleSnapshot` + `angleStats`. History stays inside the processor; the store keeps only the latest snapshot and small stats summary.                                                                                                                             |
| `src/components/pose-debug/`              | Adds `JointAngleGrid`, `AngleStatsCard`, `AngleAvailabilityCard`, and the composite `AngleDebugPanel` plus a "Log current angle snapshot" debug button.                                                                                                                               |
| `src/pages/workout/ActiveWorkoutPage.tsx` | Renders the Phase 19 `AngleDebugPanel` below the Phase 17 / 18 pose debug surface. The route is still debug-only — no final workout HUD, no reps, no scores.                                                                                                                          |

### Pipeline

For each Phase 18 `ProcessedPoseFrame` the angle processor:

1. Computes every named joint angle / metric. Each returns a typed
   `AngleValue` / `AngleMetricValue` with its own availability status —
   `available`, `key-landmarks-missing`, `key-landmarks-occluded`,
   `normalization-unavailable`, or `numeric-instability`. Unavailable
   results carry `null` values.
2. Builds the aggregate `AngleAvailabilityReport` summarizing how many
   angles / metrics came back as available and which did not.
3. Pushes the snapshot onto the bounded `AngleHistory` ring buffer.
4. Measures total per-frame overhead with `performance.now()` and
   returns `AngleCalculationResult` (snapshot + stats).

On a `null` processed frame, the processor clears its history and
returns `snapshot: null`. The hook reflects this in the store; the UI
shows "Waiting for pose" instead of a stale snapshot.

### Privacy and honesty rules (Phase 19)

- No fake angles. Missing / occluded landmarks → typed unavailable result.
- No `NaN` / `Infinity` ever reaches UI; every numeric path is clamped or short-circuited.
- No exercise thresholds, no "good vs bad" classification, no rep state, no form score, no coaching cues.
- No angle history persistence — the buffer lives in memory and resets on stop / unmount / model restart / no-pose.
- No analytics, no backend sync, no Supabase / auth / payments.

### Deferred to later phases

- Phase 20 — `SquatEngine` state machine + rep counting.
- Phase 21 — squat form rules, cue prioritization, per-exercise visibility floors.
