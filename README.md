...
- Body landmark detection, pose estimation, skeleton overlay, and form scoring (Phase 17+)
- Real-time AI coaching, rep counting, and active workout UI
...
The `src/` folders created in Phase 4 are populated only when their phase arrives. As of Phase 16, `src/theme/` holds the design-system foundation, `src/router/` holds the route table / guards / layouts / lazy loading, `src/pages/` holds route pages including onboarding, the dashboard, the Workout Library, Workout Detail, and Camera Setup, `src/hooks/` adds typed navigation, dashboard data reads, `useDebouncedValue`, `useWorkoutDetailData`, and `useCameraLightingCheck`, `src/store/` holds the onboarding draft store, `src/types/` holds onboarding, dashboard, workout-library/detail, and camera-setup domain types, `src/data/workout-library.ts` holds the canonical static MVP catalog plus workout detail sequences (real product content, not user data), `src/components/primitives/` holds the Phase 8 primitive UI library, `src/components/feedback/` holds the Phase 9 feedback / status / progress library, `src/components/camera-setup/` holds the Phase 16 setup composites, `src/components/onboarding/` holds onboarding screen components, `src/components/dashboard/` holds Phase 13 dashboard composites, `src/components/workout-library/` holds the Phase 14 library composites, `src/components/workout-detail/` holds the Phase 15 detail/pre-workout composites, `src/components/routing/` holds the Phase 6 routing-infrastructure components, `src/platform/haptics.ts` is the first platform adapter (vibration), `src/platform/onboarding-storage.ts` holds the minimal local onboarding persistence adapter, `src/platform/camera-permission.ts` holds the Phase 12 permission primer, `src/platform/camera-stream.ts` holds the Phase 16 live setup stream adapter, `src/platform/camera-lighting.ts` holds the local canvas sampler, `src/platform/speech.ts` holds the optional setup speech adapter, `src/utils/cn.ts` is the shared `clsx` wrapper, `src/utils/score.ts` defines the canonical 0–100 clamp + `scoreTone` helper, `src/utils/formatDuration.ts` formats clock-style durations, `src/utils/workout-library.ts` holds the pure filter/search/sort helpers for the library, `src/utils/workout-limitations.ts` derives gentle limitation warnings from real onboarding limitations, and `src/utils/camera-lighting.ts` classifies local brightness samples. Workout history, real workout sessions, ML/pose detection, active workout UI, authentication, payments, and later product systems are still deferred to their later phases.
