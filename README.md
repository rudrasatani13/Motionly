...
The `src/` folders created in Phase 4 are populated only when their phase arrives. As of Phase 13, `src/theme/` holds the design-system foundation, `src/router/` holds the route table / guards / layouts / lazy loading, `src/pages/` holds route pages including onboarding and the dashboard, `src/hooks/` adds typed navigation plus dashboard data reads, `src/store/` holds the onboarding draft store, `src/types/` holds onboarding and dashboard domain types, `src/components/primitives/` holds the Phase 8 primitive UI library, `src/components/feedback/` holds the Phase 9 feedback / status / progress library, `src/components/onboarding/` holds onboarding screen components, `src/components/dashboard/` holds Phase 13 dashboard composites, `src/components/routing/` holds the Phase 6 routing-infrastructure components, `src/platform/haptics.ts` is the first platform adapter (vibration), `src/platform/onboarding-storage.ts` holds the minimal local onboarding persistence adapter, `src/utils/cn.ts` is the shared `clsx` wrapper, `src/utils/score.ts` defines the canonical 0–100 clamp + `scoreTone` helper, and `src/utils/formatDuration.ts` formats clock-style durations. Workout-library data, workout history, real authentication, and later product systems are still deferred to their later phases.
